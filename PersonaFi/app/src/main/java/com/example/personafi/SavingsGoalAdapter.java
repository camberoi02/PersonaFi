...
public class SavingsGoalAdapter extends RecyclerView.Adapter<SavingsGoalAdapter.ViewHolder> {
    ...
    private List<SavingsGoal> filteredGoals;
    ...
    public SavingsGoalAdapter(List<SavingsGoal> savingsGoals, Context context, OnProgressUpdatedListener listener) {
        ...
        this.filteredGoals = new ArrayList<>(savingsGoals);
    ...
    public void updateGoals(List<SavingsGoal> newGoals) {
        ...
        filterGoals(currentSearchQuery);
    }

    public void filterGoals(String query) {
        currentSearchQuery = query.toLowerCase().trim();
        filteredGoals.clear();
        
        if (currentSearchQuery.isEmpty()) {
            filteredGoals.addAll(savingsGoals);
        } else {
            for (SavingsGoal goal : savingsGoals) {
                if (goal.getName().toLowerCase().contains(currentSearchQuery)) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (position < 0 || position >= filteredGoals.size()) return;
        
        SavingsGoal goal = filteredGoals.get(position);
        if (goal == null) return;
